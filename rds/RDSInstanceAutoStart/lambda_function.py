...
def get_message(status, body, ids=None):
    ...
    return {
        ...
        BODY_KEY: {
            **{ENV_KEY: env},
            **({IDS_KEY: ids, "StartDBInstanceResponses": body} if status == 200 else {MSG_KEY: body}),
        },
    }
...
def lambda_handler(event, context):
    ...
    if ids:
        for id_ in ids:
            rds_res = rds.start_db_instance(DBInstanceIdentifier=id_)
            logger.info(f"## RDS Start Database Instance response: {rds_res}")
    ...
    subject = f"Lambda: {os.environ['AWS_LAMBDA_FUNCTION_NAME']} started {f'{len(ids)}x' if ids else 'no'} RDS database instances"
