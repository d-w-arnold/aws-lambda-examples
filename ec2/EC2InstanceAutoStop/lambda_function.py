...
def get_message(status, body, ids=None):
    ...
    return {
        ...
        BODY_KEY: {
            **{ENV_KEY: env},
            **({IDS_KEY: ids, "StopInstancesResponse": body} if status == 200 else {MSG_KEY: body}),
        },
    }
...
def lambda_handler(event, context):
    ...
    if ids:
        ec2_res = ec2.stop_instances(InstanceIds=list(ids.keys()))
        logger.info(f"## EC2 Stop Instances response: {ec2_res}")
    body = ec2_res if ids else "No EC2 instance IDs found."
    status = 200 if not isinstance(body, str) else 404
    subject = (
        f"Lambda: {os.environ['AWS_LAMBDA_FUNCTION_NAME']} stopped {f'{len(ids)}x' if ids else 'no'} EC2 instances"
    )
